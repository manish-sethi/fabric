...
const (
	PrwsetPrefix     string = "P" // key prefix for storing private read-write set in transient store.
	PurgeIndexPrefix string = "I" // key prefix for storing index on private read-write set using endorsement block height.
)
...
func (store *transientStore) Persist(txId string, endorserId string,
	...
	compositeKey := createCompositeKeyForPRWSet(PrwsetPrefix, txId, endorserId, endorsementBlkHt)
	...
	compositeKey = createCompositeKeyForPurgeIndex(PurgeIndexPrefix, endorsementBlkHt, txId, endorserId)
...
func (store *transientStore) GetTxPrivateRWSetByTxId(txId string) (commonledger.ResultsIterator, error) {
	...
	startKey := createTxIdRangeStartKey(PrwsetPrefix, txId)
	endKey := createTxIdRangeEndKey(PrwsetPrefix, txId)
...
func (store *transientStore) Purge(maxBlockNumToRetain uint64) error {
	...
	startKey := createEndorsementBlkHtRangeStartKey(PurgeIndexPrefix, 0)
	endKey := createEndorsementBlkHtRangeEndKey(PurgeIndexPrefix, maxBlockNumToRetain-1)
	iter := store.db.GetIterator(startKey, endKey)
