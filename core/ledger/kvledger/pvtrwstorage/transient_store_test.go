...
func TestTransientStore(t *testing.T) {
	...
	var err error
	err = env.testTransientStore.Persist(txId, endorsersResults[0].EndorserId,
		endorsersResults[0].EndorsementBlockHeight, endorsersResults[0].PrivateSimulationResults)
	assert.NoError(err)

	err = env.testTransientStore.Persist(txId, endorsersResults[1].EndorserId,
		endorsersResults[1].EndorsementBlockHeight, endorsersResults[1].PrivateSimulationResults)
	assert.NoError(err)
	...
	for result, err = iter.Next(); result != nil; {
