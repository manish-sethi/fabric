...
type kvLedger struct {
	...
	transientStore transientdata.Store
}
...
func newKVLedger(ledgerID string, blockStore *ledgerstorage.Store,
	...
	transientStore transientdata.Store) (*kvLedger, error) {
...
func (l *kvLedger) Commit(block *common.Block) error {
	...
	return l.CommitWithPvtData(&ledger.PvtDataAndBlock{Block: block, BlockPvtData: pvtdata})
}

// CommitWithPvtData commits the block and the corresponding pvt data in an atomic operation
func (l *kvLedger) CommitWithPvtData(pvtdataAndBlock *ledger.PvtDataAndBlock) error {
	var err error
...
func (l *kvLedger) GetPvtDataAndBlockByNum(blockNum uint64, filter ledger.PvtNsCollFilter) (*ledger.PvtDataAndBlock, error) {
...
func retrievePrivateData(transientStore transientdata.Store, block *common.Block) (map[uint64]*ledger.TxPvtData, error) {
	...
	for txIndex, envBytes := range block.Data.Data {
		...
		pvtEndorsement, err := transientStore.GetSelfSimulatedTxPvtRWSet(chdr.TxId)
		if err != nil {
			return nil, err
		}
		...
		if err := proto.Unmarshal(pvtEndorsement.PrivateSimulationResults, txPvtRWSet); err != nil {
			return nil, err
		}
