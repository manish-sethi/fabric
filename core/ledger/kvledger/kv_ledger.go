...
type kvLedger struct {
	...
	transientStore pvtrwstorage.TransientStore
}
...
func newKVLedger(ledgerID string, blockStore blkstorage.BlockStore,
	...
	transientStore pvtrwstorage.TransientStore) (*kvLedger, error) {
...
// GetPrivateSimulationResults returns a private simulation results for a given transaction id
func (l *kvLedger) GetPrivateSimulationResults(txId string) (commonledger.ResultsIterator, error) {
	return l.transientStore.GetTxPrivateRWSetByTxId(txId)
}

// PutPrivateSimulationResults stores a private simulation results generated by an endorser as a
// result of transaction simulation
func (l *kvLedger) PutPrivateSimulationResults(txId string, endorserId string, privateSimulationResults []byte) error {
	// Endorser should return the readLock only after storing the private write set in the
	// transient store.
	blockchainInfo, err := l.blockStore.GetBlockchainInfo()
	if err != nil {
		return err
	}
	return l.transientStore.Persist(txId, endorserId, blockchainInfo.Height, privateSimulationResults)
}

// Purge removes private read-writes set generated by endorsers at block height lesser than
// a given minBlockNumToRetain. In other words, Purge only retains private read-write sets
// that were generated at block height of minBlockNumToRetain or higher.
func (l *kvLedger) PurgeTransientData(minBlockNumToRetain uint64) error {
	return l.transientStore.Purge(minBlockNumToRetain)
}

// TempDataMinBlockNum returns the lowest retained endorsement block height
func (l *kvLedger) TransientDataMinBlockNum() (uint64, error) {
	return l.transientStore.GetMinEndorsementBlkHt()
}
