...
// GetTransientData returns a private simulation results for a given transaction id
func (l *kvLedger) GetTransientData(txID string) (commonledger.ResultsIterator, error) {
	return l.transientStore.GetTxPrivateRWSetByTxid(txID)
}

// PutTransientData stores a private simulation results generated by an endorser as a
// result of transaction simulation. See interface 'ledger.PeerLedger' for more detailed comments
func (l *kvLedger) PutTransientData(txID string, endorserID string, blockHeight uint64,
	privateSimulationResults []byte) error {
	return l.transientStore.Persist(txID, endorserID, blockHeight, privateSimulationResults)
}
