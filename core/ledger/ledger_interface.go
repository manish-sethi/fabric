...
type PeerLedger interface {
	...
	// GetTransientData returns a private simulation results for a given txid
	// this function is expected to be used by gossip in phase-2 of the sidedb when an another peer requests
	// the private simulation results for validation and commit
	GetTransientData(txid string) (commonledger.ResultsIterator, error)
	// PutTransientData stores a private simulation results generated by an endorser as a
	// This function is expected to be called by gossip in phase-2 of the sidedb. In the phase-1, it is assumed
	// that each endorser is involved in the endorsement process and hence does not need the private data from other endorsers
	PutTransientData(txid, endorserid string, blockHeight uint64, privateSimulationResults []byte) error
	// PurgeTransientData removes private read-writes set generated by endorsers at block height lesser than
	...
	PurgeTransientData(maxBlockNumToRetain uint64) error
...
type HistoryQueryExecutor interface {
	...
	// GetHistoryForPrivateData retrieves the history of values for a key in private data space.
	// The returned ResultsIterator contains results of type *KeyModification which is defined in protos/ledger/queryresult.
	GetHistoryForPrivateData(namespace, collection, key string) (commonledger.ResultsIterator, error)
}
