...
type PeerLedger interface {
	...
	GetPvtDataAndBlockByNum(blockNum uint64, collections []string) (*PvtDataAndBlock, error)
	// GetPvtDataByNum returns only the pvt data  corresponding to the given block number
	// The pvt data is filtered by the list of 'collections' supplied
	GetPvtDataByNum(blockNum uint64, collections []string) ([]*TxPvtData, error)
	// CommitWithPvtData commits the block and the corresponding pvt data in an atomic operation
	CommitWithPvtData(PvtDataAndBlock *PvtDataAndBlock) error
...
// TxPvtData encapsulates the transaction id and pvt write-set for a transaction
type TxPvtData struct {
	Txid     string
	WriteSet *rwset.TxPvtReadWriteSet
}

// PvtDataAndBlock encapsultes the block and a map that contains the tuples <txid, *TxPvtData>
type PvtDataAndBlock struct {
	Block        *common.Block
	BlockPvtData map[string]*TxPvtData
}
