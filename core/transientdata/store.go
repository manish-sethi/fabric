...
type Store interface {
	...
	GetSelfSimulatedTxPvtRWSet(txid string) (*EndorserPrivateSimulationResults, error)
...
// GetTxPrivateRWSetByTxid returns private read-write set of a given transaction
func (s *store) GetTxPrivateRWSetByTxid(txid string) (commonledger.ResultsIterator, error) {
...
// GetSelfSimulatedTxPvtRWSet returns the pvt read-write set from the simulation performed by the peer itself
func (s *store) GetSelfSimulatedTxPvtRWSet(txid string) (*EndorserPrivateSimulationResults, error) {
	var err error
	var itr commonledger.ResultsIterator
	if itr, err = s.GetTxPrivateRWSetByTxid(txid); err != nil {
		return nil, err
	}
	defer itr.Close()
	var temp commonledger.QueryResult
	var res *EndorserPrivateSimulationResults
	for {
		...
		res = temp.(*EndorserPrivateSimulationResults)
...
func (scanner *rwsetScanner) Next() (commonledger.QueryResult, error) {
	...
	return &EndorserPrivateSimulationResults{
...
func selfSimulated(pvtSimRes *EndorserPrivateSimulationResults) bool {
